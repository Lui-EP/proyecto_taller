...
const trustBullets = [
  'Registro y autenticación centralizada con JWT',
  'Microservicios escalables por dominio',
  'Operación web y móvil conectada por API',
];
...
export default function LandingPage() {
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-gradient-to-b from-[#FBF5EA] via-[#F7EEDC] to-[#EEDFC5] text-[#4B3217]">
      ...
      <header className="sticky top-0 z-30 border-b border-[#DFC7A0]/70 bg-[#FDF7EC]/85 backdrop-blur-xl">
        <div className="mx-auto flex w-full max-w-6xl items-center justify-between px-6 py-4">
          ...
          <a
            href={registerUrl}
            className="inline-flex items-center justify-center rounded-xl border border-[#D9BE94] bg-white px-4 py-2 text-sm font-bold text-[#A86A2A] transition hover:-translate-y-0.5 hover:bg-[#F9F1E2]"
          >
            Registrarme
          </a>
        </div>
      ...
      <section className="relative z-10 mx-auto w-full max-w-6xl px-6 pb-10 pt-10 md:pt-14">
        <div className="grid items-center gap-8 rounded-[2rem] border border-[#D9BE94] bg-white/80 p-8 shadow-[0_24px_70px_-30px_rgba(168,106,42,0.45)] backdrop-blur transition-all duration-500 hover:shadow-[0_26px_75px_-28px_rgba(168,106,42,0.55)] md:grid-cols-[1.1fr_0.9fr] md:p-12">
          <div>
            ...
            <SplitText
              as="h1"
              text="JAGUARYU: El futuro de tu negocio en la nube"
              className="max-w-3xl text-balance text-3xl font-black leading-tight text-[#4B3217] md:text-5xl"
              delay={40}
              stagger={34}
            />
            ...
            <div className="mt-6 grid gap-2">
              {trustBullets.map((line) => (
                <div key={line} className="flex items-center gap-2 text-sm text-[#6B4A27]">
                  <span className="grid h-5 w-5 place-items-center rounded-full bg-[#EED7B1] text-[11px] font-black text-[#8D571F]">✓</span>
                  <span>{line}</span>
                </div>
              ))}
            </div>
          ...
          <aside className="grid gap-3 rounded-3xl border border-[#E4CFAB] bg-gradient-to-br from-[#FEFAF2] to-[#F2E2C4] p-5 transition-all duration-500 hover:-translate-y-1">
            ...
            <div className="mt-1 rounded-2xl border border-dashed border-[#D8BB8E] bg-[#FFF8EB] p-4">
              <p className="text-xs uppercase tracking-wide text-[#A86A2A]">URL principal</p>
              <p className="mt-1 break-all text-sm font-semibold text-[#6B4A27]">{registerUrl}</p>
            </div>
          </aside>
        </div>
      </section>
      ...
      <section className="mx-auto w-full max-w-6xl px-6 pb-14">
        <div className="mb-6 flex flex-wrap items-end justify-between gap-3">
          <SplitText
            as="h2"
            text="Microservicios principales"
            className="text-2xl font-extrabold text-[#4B3217] md:text-3xl"
            delay={10}
            stagger={24}
          />
      ...
      <section className="mx-auto w-full max-w-6xl px-6 pb-24">
        <div className="rounded-[2rem] border border-[#D9BE94] bg-white/80 p-6 shadow-[0_18px_42px_-28px_rgba(168,106,42,0.45)] md:p-10">
          <SplitText
            as="h2"
            text="Base técnica de la plataforma"
            className="text-2xl font-extrabold text-[#4B3217] md:text-3xl"
            delay={10}
            stagger={22}
          />
          <p className="mt-3 max-w-3xl text-sm leading-relaxed text-[#6B4A27] md:text-base">
            JAGUARYU opera sobre una arquitectura desacoplada orientada a escalabilidad, con despliegue
